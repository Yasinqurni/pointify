...
async function main() {
    ...
    try {
        ...
        const isRegisteredBefore = await treasuryManager.isApprovedMerchant(merchant.address);
        ...
        if (merchantPLTCurrent >= rewardAmount) {
            // Approve PLT spending by TreasuryManager
            console.log("   • Approving PLT spending...");
            const approvePLTTx = await pointifyToken.connect(merchant).approve(TREASURY_MANAGER_ADDRESS, rewardAmount);
            await approvePLTTx.wait();
            console.log("   ✅ PLT approved");
        ...
        if (userPLTCurrent >= redeemAmount) {
            // Approve PLT spending by TreasuryManager
            console.log("   • Approving PLT spending...");
            const approveUserPLTTx = await pointifyToken.connect(user).approve(TREASURY_MANAGER_ADDRESS, redeemAmount);
            await approveUserPLTTx.wait();
            console.log("   ✅ PLT approved");
