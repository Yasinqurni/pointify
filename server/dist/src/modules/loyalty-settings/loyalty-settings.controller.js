"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LoyaltySettingsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const jwt_auth_guard_1 = require("../../common/jwt-auth.guard");
const loyalty_settings_service_1 = require("./loyalty-settings.service");
const loyalty_settings_dto_1 = require("../../dto/loyalty-settings.dto");
let LoyaltySettingsController = class LoyaltySettingsController {
    loyaltySettingsService;
    constructor(loyaltySettingsService) {
        this.loyaltySettingsService = loyaltySettingsService;
    }
    async getLoyaltySettings(req) {
        return this.loyaltySettingsService.getLoyaltySettings(req.user.userId);
    }
    async updateLoyaltySettings(req, updateDto) {
        return this.loyaltySettingsService.updateLoyaltySettings(req.user.userId, updateDto);
    }
};
exports.LoyaltySettingsController = LoyaltySettingsController;
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Get merchant loyalty settings' }),
    (0, swagger_1.ApiResponse)({
        status: 200,
        description: 'Loyalty settings retrieved successfully',
        type: loyalty_settings_dto_1.LoyaltySettingsResponseDto,
    }),
    __param(0, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], LoyaltySettingsController.prototype, "getLoyaltySettings", null);
__decorate([
    (0, common_1.Put)(),
    (0, swagger_1.ApiOperation)({ summary: 'Update merchant loyalty settings' }),
    (0, swagger_1.ApiResponse)({
        status: 200,
        description: 'Loyalty settings updated successfully',
        type: loyalty_settings_dto_1.LoyaltySettingsResponseDto,
    }),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, loyalty_settings_dto_1.UpdateLoyaltySettingsDto]),
    __metadata("design:returntype", Promise)
], LoyaltySettingsController.prototype, "updateLoyaltySettings", null);
exports.LoyaltySettingsController = LoyaltySettingsController = __decorate([
    (0, swagger_1.ApiTags)('Loyalty Settings'),
    (0, common_1.Controller)('loyalty-settings'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, swagger_1.ApiBearerAuth)(),
    __metadata("design:paramtypes", [loyalty_settings_service_1.LoyaltySettingsService])
], LoyaltySettingsController);
//# sourceMappingURL=loyalty-settings.controller.js.map