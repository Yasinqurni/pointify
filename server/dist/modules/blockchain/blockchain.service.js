...
let BlockchainService = BlockchainService_1 = class BlockchainService {
    ...
    async rewardUser(merchantWallet, userWallet, amount) {
        try {
            ...
            const tx = await rewardManagerWithSigner.rewardUser(userWallet, amountWei);
            await tx.wait();
    ...
    async redeemPoint(userWallet, amount) {
        try {
            ...
            const amountWei = ethers_1.ethers.parseEther(amount);
            const tx = await redemptionRouterWithSigner.redeemPoint(amountWei);
            await tx.wait();
    ...
    async manualRedeem(userWallet, amount, item, merchantWallet) {
        try {
            ...
            const redemptionRouterWithSigner = this.redemptionRouterContract.connect(signer);
            const amountWei = ethers_1.ethers.parseEther(amount);
            const tx = await redemptionRouterWithSigner.manualRedeem(userWallet, amountWei, item, merchantWallet);
            await tx.wait();
    ...
    async swapPLTForToken(tokenAddress, pltAmount) {
        try {
            ...
            const amountWei = ethers_1.ethers.parseEther(pltAmount);
            const tx = await swapRouterWithSigner.swapPLTForToken(tokenAddress, amountWei);
            await tx.wait();
    ...
    async swapTokenForPLT(tokenAddress, tokenAmount) {
        try {
            const signer = await this.provider.getSigner();
            const swapRouterWithSigner = this.swapRouterContract.connect(signer);
            const amountWei = ethers_1.ethers.parseEther(tokenAmount);
            const tx = await swapRouterWithSigner.swapTokenForPLT(tokenAddress, amountWei);
            await tx.wait();
